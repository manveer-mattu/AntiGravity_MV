...
export function ReviewCard({ review, onStatusChange, onDismiss }: ReviewCardProps) {
    ...
    // Click outside handler for dismissal when in saved state
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (showSavedConfirmation && cardRef.current && !cardRef.current.contains(event.target as Node)) {
                if (onDismiss) {
                    onDismiss();
                }
            }
    ...
    }, [showSavedConfirmation, onDismiss]);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (data.reply) {
                ...
                // NOTE: We no longer auto-save to DB or update parent status here.
                // It stays local until "Save Draft" or "Post" is clicked.
            }
        } catch (error) {
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            if (onStatusChange) onStatusChange('draft', draftText, isFallback);

            // NOTE: Timeout removed. Confirmation stays until dismissed by clicking outside.
        } finally {
            setIsSaving(false);
        }
    };
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex gap-2">
                                            <Button
                                                ...
                                                variant="outline"
                                                className={cn(
                                                    showSavedConfirmation && "text-green-600 border-green-600 hover:text-green-700 hover:bg-green-50"
                                                )}
                                                onClick={async () => {
                                                    setIsEditing(false);
                                                ...
                                                disabled={isSaving}
                                            >
                                                {isSaving ? "Saving..." : showSavedConfirmation ? (
                                                    <>
                                        ...
                                        {status === 'draft' && (
                                            <div className="flex gap-2">
                                                ...
                                                <Button size="sm" variant="outline" onClick={async () => {
                                                    ...
                                                    // Reset in DB - only if it was already saved? 
                                                    // If it wasn't saved, this call is redundant but harmless.
                                                    // If it WAS saved (e.g. from previous session), we should clear it.
                                                    await fetch('/api/save-reply', {
                                                        method: 'POST',
                                                        headers: { 'Content-Type': 'application/json' },
                                                        body: JSON.stringify({
