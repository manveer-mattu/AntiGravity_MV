...
interface DashboardClientProps {
    ...
    initialStats: DailySentimentStat[];
}

export function DashboardClient({ initialReviews, initialStats }: DashboardClientProps) {
    const [reviews, setReviews] = useState<GoogleReview[]>(initialReviews);
    const [stats] = useState<DailySentimentStat[]>(initialStats);
    ...
    // Progressive AI enrichment — fires on mount to upgrade reviews with sentiment/topics
    useEffect(() => {
        import('@/app/actions/analysis').then(({ enrichReviewsWithAI }) => {
            enrichReviewsWithAI(initialReviews).then(enrichedReviews => {
